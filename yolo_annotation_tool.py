...
import threading
import csv
from recognize_objects import ObjectRecognizer
...
ob = ObjectRecognizer(
    "./models/yolov3.weights", "./models/yolov3.cfg", "./models/coco.names"
)
...
def get_one_image(list_of_images, annotation_index) -> NDArray:
    return cv2.imread(list_of_images[annotation_index])


def show_image(frame: NDArray):
    cv2.imshow("Annotation Tool", frame)
    cv2.waitKey(0)


def list_of_labels() -> ty.List[str]:
    if all_labels == []:
...
def get_label() -> str:
    ...
    return os.popen(
        'zenity --list  --height 500 --title="Labels" --column="Index" --column="Label"'
        + labels
    ).read()


def pre_annotate(frame: NDArray, all_labels: ty.List[str]):
    ob_results = ob.process_frame(frame, all_labels, False)
    return ob_results


def convert_list_to_str(lst: ty.List[str]):
    return str(lst).translate(None, "[],'")


def convert_boxes_to_yolo(yolo_boxes: NDArray, frame: NDArray):
    yolo_label = []
    for name, box in yolo_boxes:
        ...
        yolo_label.append(
            " ".join([str(all_labels.index(name)), str(xc), str(yc), str(wc), str(hc)])
        )
    return yolo_label


def save_annotation(frame: NDArray, labels: ty.List[str]):
    ...
    while keep_running != 0:
        for name, boxes in yolo_boxes:
            cv2.rectangle(
                ...
                (boxes[0][0], boxes[0][1]),
                (boxes[1][0], boxes[1][1]),
                colors[list_of_labels().index(name)],
                2,
            )
            cv2.putText(
                frame,
        ...
        if (
            mouse_point[0] > 0
            and mouse_point[0] < frame_temp.shape[1]
            and mouse_point[1] > 0
            and mouse_point[1] < frame_temp.shape[0]
        ):
            cv2.line(
                frame_temp,
                (mouse_point[0], 0),
                (mouse_point[0], frame_temp.shape[0]),
                (0, 255, 255),
                1,
            )
            cv2.line(
                frame_temp,
                (0, mouse_point[1]),
                (frame_temp.shape[1], mouse_point[1]),
                (0, 255, 255),
                1,
            )
...
if __name__ == "__main__":
    ...
    for im in range(len(list_of_images)):
        ...
        yolo_boxes = pre_annotate(
            frame, ["person", "car", "cat", "dog", "cow", "traffic light"]
        )  # list_of_labels())
